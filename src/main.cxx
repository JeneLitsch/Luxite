...
std::vector<std::uint8_t> generate_image(stx::size2u size, const Scene & scene) {
	...
	constexpr static stx::vector3f start {-3,-3,3};
	...
	for(std::int32_t y = 0; y < size.y; ++y){
		for(std::int32_t x = 0; x < size.x; ++x){
			...
			const stx::vector3f dir {dx,dy, -0.01};
			// std::cout << dir << " | \n";
			// std::cout << dx << " | \n";
			
			auto end = ray_cast(start, dir, [&] (stx::position3i coord) {
				...
				if(v.a != 0) {
					data[(x + (y * size.x)) * 4 + 0] = v.r;
					data[(x + (y * size.x)) * 4 + 1] = v.g;
					data[(x + (y * size.x)) * 4 + 2] = v.b;
					return false;
				}
				return true;
			});
